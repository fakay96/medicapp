...
router.get('/', isLoggedInAsStudent, function(req, res) {
    ...
    res.render('exams/takepiccode', {title: "Enter Exam Code", username: req.user.username});
                ...
                else if(doc)
                {
                    Exam.checkResponse(username, exam_code, function(err, doc)
                    {
                        if(err)
                            ...
                            res.redirect('/takepic');
                        else
                        {
                            res.render('exams/takepic_exam', { title: 'Take Exam', exam_code: exam_code, username: username});
                        }
                    })
                }
                else
